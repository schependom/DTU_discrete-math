...
Because of this fact, we conclude that **a field is necessarily an integral domain** (free of zero-divisors):
...
In general, a polynomial is **uniquely determined by its coefficients**. In other words, two polynomials $p(X) = \sum_{i=0}^d r_i X^i$ and $q(X) = \sum_{i=0}^e s_i X^i$ are equal if and only if $d = e$ and $r_i = s_i$ for all $i = 0, 1, \ldots, d$.
...
>
> Then
> $$\boxed{p(X) = q(X) \iff \forall i \in \{0, 1, \ldots, \infty\} : r_i = s_i}.$$
...
**Now, it is sufficient to check if $p(a) \equiv 0 \pmod n$!**

Why? We saw that $\equiv \pmod n$ is an equivalence relation on $\Z$. The standard representatives of the equivalence classes are the elements of $\Z_n = \{0, 1, \ldots, n-1\}$. We have that
$$\forall x \in \Z : x \equiv x \pmod n$$
So also $0 \equiv 0 \pmod n$. Therefore, to check if $p(a) = 0$ in $\Z_n$, it is sufficient to check if $p(a) \equiv 0 \pmod n$ in $\Z$, because the standard representative is unique.
...
> Let $(D, +, \cdot)$ be an integral **domain** and let $p(X) \in D[X]$ be a non-zero polynomial. Then $$\boxed{\deg(p(X)) = n \geq 0 \implies p(X) \text{ has at most $n$ distinct roots in $D$}}$$
