...
> [!deduction] $\text{fields} \subseteq \text{integral domains}$
>
> $$
...
> Let $(R, +, \cdot)$ be a commutative ring and let $p(X), q(X) \in R[X]$ be two polynomials different from zero. Assume that $d(X)$ is **<u>monic</u>** (i.e. its leading coefficient is $1_R$). Then there exist polynomials $q(X), r(X) \in R[X]$ such that
>
...
$$\implies \boxed{\textbf{it is sufficient to check if } p(a) \equiv 0 \pmod n}$$
...
> Let $(D, +, \cdot)$ be an integral **domain** (or a field) and let $p(X) \in D[X]$ be a non-zero polynomial. Then $$\boxed{\deg(p(X)) = n \geq 0 \implies p(X) \text{ has at most $n$ distinct roots in $D$}}$$
